      ...
      <div className="space-y-6">
        ...
        {savedAudio.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-500">Generate some audio and save it to see it appear here</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {savedAudio.map((audio) => (
              <div key={audio.id} className="p-4 bg-gray-800/50 border border-gray-700 rounded-xl space-y-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-600">{formatDate(audio.created_at)}</p>
                  </div>
                  <button
                    type="button"
                ...
                <audio controls src={audioService.getAudioUrl(audio.filename)} className="w-full" style={{ height: '40px' }} />

                <div className="flex gap-2">
                  <Button variant="secondary" size="sm" fullWidth onClick={() => handleDownload(audio)}>
        ...
        {savedAudio.length > 0 && totalLabel}
      </div>
    </Card>
  );
};