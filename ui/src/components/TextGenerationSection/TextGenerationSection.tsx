...
export const TextGenerationSection: React.FC = () => {
    ...
    const { voices, isLoadingVoices, refreshVoices } = useVoices();

    const pollGenerationStatus = async (taskId: string) => {
        const interval = setInterval(async () => {
            try {
                const response = await audioService.checkGenerationStatus(taskId);
                if (response.status === 'completed' && response.result_path) {
                    clearInterval(interval);
                    setAudioUrl(audioService.getAudioUrl(response.result_path));
                    setIsGenerating(false);
                } else if (response.status === 'failed') {
                    clearInterval(interval);
                    setGenerationError(response.error || 'Audio generation failed.');
                    setIsGenerating(false);
                }
            } catch (error) {
                clearInterval(interval);
                setGenerationError('Failed to get generation status.');
                setIsGenerating(false);
    ...
    const handleGenerateAudio = async () => {
        ...
        try {
            const taskId = await audioService.startSpeechGeneration(text, selectedVoice.id);
            pollGenerationStatus(taskId);
        } catch (error) {
            setGenerationError('Failed to start audio generation task.');
                ...
                <div className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            <textarea
                                ...
                                rows={10}
                                placeholder="Enter or paste your text here. Supports long-form content..."
                                value={text}
                                onChange={(e) => setText(e.target.value)}
                                disabled={isGenerating}
                            />
                            <div className="absolute bottom-2 right-2 text-xs text-gray-500">{text.length} characters</div>
                        </div>
                    </div>
                    ...
                    {isGenerating && (
                        <div className="p-4 bg-blue-900/20 border border-blue-800/50 rounded-xl">
                            <p className="text-sm text-blue-300">Generating audio... This may take a moment for long texts.</p>
                        </div>
                    )}
                    {generationError && (
